...
exports.createSauce = async (req, res) => {
  ...
  if (userLikedOrDisliked.length > 0) {
    return res
      .status(400)
      .json({ message: "UsersLiked and UsersDisliked has to be empty" });
  }
...
exports.likeAndDislike = async (req, res) => {
  ...
  const goodLikeStatus =
    likeStatus === 1 || likeStatus === -1 || likeStatus === 0;
  if (!goodLikeStatus) {
    ...
    try {
      ...
      if (sauce.usersLiked.includes(userId)) {
        ...
        return res.status(400).json({
          message:
            "You need to decreased your dislike before increased the like",
        });
      }
  ...
  // If the user decides to remove their like or dislike
    ...
    try {
      ...
      if (sauce.usersLiked.includes(userId)) {
        ...
        res
          .status(400)
          .json({ message: "You have neither liked nor disliked" });
      }
    } catch (error) {
      res.status(400).json({ message: "Non-existent sauce id" });
    }
  ...
  //If user don't like the sauce
    ...
    try {
      ...
      if (sauce.usersDisliked.includes(userId)) {
        ...
        return res.status(400).json({
          message:
            "You need to decreased your like before increased the dislike",
        });
      }
      await Sauce.updateOne(
        { _id: req.params.id },
